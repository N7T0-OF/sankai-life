...
object MemoEngine {
    ...
    /**
     * Horaires d'envoi d'un module pour la journée.
     *
     * La première notification tombe à l'heure choisie par l'utilisateur, les
     * suivantes sont réparties toutes les `12 / fréquence` heures. Avec 1/jour
     * on obtient exactement l'heure demandée ; avec 3/jour on obtient
     * heure, heure+4h, heure+8h. Les créneaux qui déborderaient sur la nuit
     * sont ramenés dans la journée pour ne pas réveiller l'utilisateur.
     */
    fun slotsDuJour(heure: Int, minute: Int, frequenceParJour: Int): List<Pair<Int, Int>> {
        val frequence = frequenceParJour.coerceIn(1, 6)
        val pas = 12 / frequence
        return (0 until frequence).map { index ->
            val h = (heure + index * pas)
            // Au-delà de 22h on replie sur le matin plutôt que sur la nuit.
            val heureFinale = if (h > 22) ((h - 22) + 8).coerceAtMost(22) else h
            heureFinale to minute
        }.distinct()
    }
}
